...
void conver_enum_to_text(PERSONAL_BELONGINGS item_name)
{
  switch (item_name)
  {
    { case SHIRT: printf("shirt"); break; };
    { case DRESS: printf("dress"); break; };
    { case BAG: printf("bag"); break; };
    { case RING: printf("ring"); break; };
    { case BRACELET: printf("bracelet"); break; };
    { case SHOES: printf("shoes"); break; };
    { case GLASSES: printf("glasses"); break; };
  }
}
...
void show_item(uint8_t *CART)
{
  ...
  if ((*CART &= SHIRT) == SHIRT) { printf("shirt "); } *CART = Temp;;
  if ((*CART &= TROUSERS) == TROUSERS) { printf("trousers "); } *CART = Temp;;
  if ((*CART &= DRESS) == DRESS) { printf("dress "); } *CART = Temp;;
  if ((*CART &= BAG) == BAG) { printf("bag "); } *CART = Temp;;
  if ((*CART &= BRACELET) == BRACELET) { printf("bracelet "); } *CART = Temp;;
  if ((*CART &= SHOES) == SHOES) { printf("shoes "); } *CART = Temp;;
  if ((*CART &= GLASSES) == GLASSES) { printf("glasses "); } *CART = Temp;;
  if ((*CART &= RING) == RING) { printf("ring "); } *CART = Temp;;
}
...
void check_item_for_cart(uint8_t *CART, PERSONAL_BELONGINGS item_name)
{
  ...
  printf(">>> Check the ");
  conver_enum_to_text(item_name);
  if ((*CART &= SHIRT) == SHIRT) { printf("-> The cart has shirt "); };
  if ((*CART &= TROUSERS) == TROUSERS) { printf("-> The cart has trousers "); };
  if ((*CART &= DRESS) == DRESS) { printf("-> The cart has dress "); };
  if ((*CART &= BAG) == BAG) { printf("-> The cart has bag "); };
  if ((*CART &= BRACELET) == BRACELET) { printf("-> The cart has brecelet "); };
  if ((*CART &= SHOES) == SHOES) { printf("-> The cart has shoes "); };
  if ((*CART &= GLASSES) == GLASSES) { printf("-> The cart has glasses "); };
  if ((*CART &= RING) == RING) { printf("-> The cart has ring "); };
  *CART = Temp;
}
int main(void)
{
